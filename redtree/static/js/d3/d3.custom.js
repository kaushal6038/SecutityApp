$(document).ready(function() {
  $.ajax({
    ...
    success: function(response) {
      ...
      pl_activity = response.pl_activity;
      vulnerability_found = response.vulnerability_found;
      burp_error = response.burp_error;
      nessus_error = response.nessus_error;
      masscan_error = response.masscan_error;
      vul = response.vul;
    }
  });
  // console.log(vul)
...
$(document).ready(function() {
	...
	last_index = event_date.length-1;
    var start_date = event_date[1];
	var end_date = event_date[last_index]
	var chart4 = c3.generate({
		...
		data: {
			...
			names: {
				data1: 'Activity',
				data3: 'Vulnerability Found',
				data4: 'Burp Errors',
				data5: 'Nessus Errors',
				data6: 'Masscan Errors',
			},
        ...
        x: {
            type: 'timeseries',
            tick: {
            	//count: 4, // we can set count number of data we want to display 
            	fit: true,
     			values: [start_date, end_date], // for showing  data this datre to this 
                format: '%Y-%m-%d'
                //format: function (x) { return x.getFullYear(); } //for Full year of data
